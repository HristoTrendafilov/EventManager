...
export interface UserProfileOrganization {
  ...
  organizationLogoRelativePath: string;
  organizationLogoUrl: string;
  organizationSubscriptionCreatedOnDateTime: Date;
...
export interface OrganizationForUpdate {
  organizationLogoUrl: string;
  organizationManagers: UserPreview[];
  organizationManagersIds: number[] | null;
  organizationName: string;
  organizationDescription: string;
}
...
export const UserNewSchema = z.object({
  ...
  userRegionsHelpingIds: z
    ...
    .int()
    .array()
    .min(1, { message: 'Изберете поне един регион, в коойто искате да помагате' }),
...
export const OrganizationBaseFormSchema = z.object({
  ...
  organizationManagersIds: z.number().int().array().nullable(),
});
...
export const OrganizationNewSchema = z.object({
  organizationLogoFile: z
    .instanceof(FileList)
    .refine((fileList) => fileList && fileList.length > 0, { message: 'Логото на организацията е задължително' })
    .refine(
      (fileList) => {
        if (fileList && fileList.length > 0) {
          return Array.from(fileList).every((file) => file.size <= 1048576);
        }
        return true;
      },
      { message: 'Максималният размер за файл е 1MB' }
    ),
  organizationName: z.string().min(1, { message: 'Името на организацията е задължително' }),
  organizationDescription: z.string().min(1, { message: 'Описанието на организацията е задължително' }),
  organizationManagersIds: z.number().int().array().nullable(),
...
export const OrganizationUpdateSchema = z.object({
  organizationLogoFile: z.instanceof(FileList).refine(
    (fileList) => {
      if (fileList && fileList.length > 0) {
        return Array.from(fileList).every((file) => file.size <= 1048576);
      }
...
export const EventBaseFormSchema = z.object({
  ...
  eventStartDateTime: z.coerce
    .date()
    .refine((date) => date >= new Date('1971-01-01') && date <= new Date('3000-01-01'), {
      message: 'Дата на събитието е задължителна.',
    }),
  regionId: z
    .number()
    .int()
    .min(1, { message: 'Регионът на събитието е задължителен.' })
    .max(9.223372036854776e18, { message: 'Регионът на събитието е задължителен.' }),
  organizationId: z
    .number()
    .int()
    .min(1, { message: 'Изберете организация или N/A.' })
    .max(9.223372036854776e18, { message: 'Изберете организация или N/A.' }),
